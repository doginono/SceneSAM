...
class Segmenter(object):
    ...
    def __init__(self, cfg, args, store_directory):
        ...
        self.semantic_frames = torch.from_numpy(
            np.zeros((self.n_img // self.every_frame, self.H, self.W))
        ).int()
    ...
    def segment_first(self):
        ...
        samplesFromCurrent = backproject.sample_from_instances_with_ids(
            ids, self.new_id, points_per_instance=10
        )
    ...
    def run(self):
        if self.use_stored:
            ...
            for index in tqdm(index_frames, desc="Loading stored segmentations"):
                ...
                self.semantic_frames[index // self.every_frame] = torch.from_numpy(
        ...
        if self.mask_generator:
            while True:
                ...
                while (
                    self.idx.item() > self.idx_mapper.item()
                    or self.idx.item() > self.idx_coarse_mapper.item()
                ):
                    time.sleep(0.1)
            ...
            if self.store_vis:
                for index in tqdm(
                    [0] + list(index_frames), desc="Storing visualizations"
                ):
                    path = os.path.join(self.store_directory, f"seg_{index}.png")
                    self.visualizer.visualize(
                        self.semantic_frames[index // self.every_frame].numpy(),
                        path=path,
                    )
