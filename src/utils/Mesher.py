...
class Mesher(object):
    def __init__(
        ...
        self.color_mesh_extraction_method = cfg["meshing"][
            "color_mesh_extraction_method"
        ]
    ...
    def point_masks(
        ...
        for i, pnts in enumerate(
            ...
            if get_mask_use_all_frames:
                for i in range(0, idx + 1, 1):  # undone: changed from 1 to 5
                ...
                for keyframe in keyframe_dict:
                    ...
                    if self.depth_test:
                        ...
                        # forecast
                        cur_mask_forecast = cur_mask_forecast.reshape(-1)
                        proj_depth_forecast = -cam_cord[cur_mask_forecast, 2].reshape(
                            -1
                        )
                        cur_mask_forecast[cur_mask_forecast.clone()] &= (
                            proj_depth_forecast < max_depth
                        )
                        # seen
                        cur_mask_seen = cur_mask_seen.reshape(-1)
    ...
    def get_mesh(
        ...
        """
        assert (color and ~semantic) or (
            ~color and semantic
        ), "color and semantic mesh extraction are mutually exclusive"
        with torch.no_grad():
            ...
            if show_forecast:
                ...
                z_forecast = []
                for i, pnts in enumerate(
                    torch.split(forecast_points, self.points_batch_size, dim=0)
                ):
                    z_forecast.append(
                        self.eval_points(pnts, decoders, c, "coarse", device)
                        .cpu()
                        .numpy()[:, -1]
                    )
                ...
                z_seen = []
                for i, pnts in enumerate(
                    torch.split(seen_points, self.points_batch_size, dim=0)
                ):
                    z_seen.append(
                        self.eval_points(pnts, decoders, c, "fine", device)
                        .cpu()
                        .numpy()[:, -1]
                    )
            ...
            if clean_mesh:
                if show_forecast:
                    ...
                    contain_mask = []
                    for i, pnts in enumerate(
                        np.array_split(points, self.points_batch_size, axis=0)
                    ):
                    ...
                    seen_mask, forecast_mask, unseen_mask = self.point_masks(
                        ...
                        device=device,
                        get_mask_use_all_frames=get_mask_use_all_frames,
                    )  # J: returns the masks for seen, forecast and unseen points during training
                ...
                if self.get_largest_components:
                    ...
                    for comp in components:
                        if (
                            comp.area
                            > self.remove_small_geometry_threshold
                            * self.scale
                            * self.scale
                        ):
                ...
                if self.color_mesh_extraction_method == "direct_point_query":
                    ...
                    for i, pnts in enumerate(
                        ...
                        z_color = self.eval_points(
                            pnts.to(device).float(), decoders, c, "color", device
                        ).cpu()[..., :3]
                        z.append(z_color)
                    ...
                    rays_o = torch.from_numpy(
                        vertices + sign * length * vertex_normals
                    ).to(device)
                    color_list = []
            ...
            # ----------------------------------added for semantic mesh extraction----------------------------------
                ...
                if self.color_mesh_extraction_method == "direct_point_query":
                    ...
                    for i, pnts in enumerate(
                        ...
                        z_semantic = self.eval_points(  # TODO eval in stage semantic
                            pnts.to(device).float(), decoders, c, "semantic", device
                        ).cpu()
                    ...
                    assert (
                        False
                    ), "render_ray_along_normal not implemented for semantic mesh extraction"
                    # for imap*
                    # render out the color of the ray along vertex normal, and assign it to vertex color
                    import open3d as o3d
                ...
                vertex_colors = vertex_colors.astype(
                    np.uint8
                )  # J: name it to colors to be able to reuse the end of the function
