...
def psnr_hierarchy(image_pred, image_gt, valid_mask=None, reduction="mean"):
    ...
    if "rgb_coarse" in image_pred:
        psnr_coarse = -10 * torch.log10(
            mse(image_pred["rgb_coarse"].detach(), image_gt, valid_mask, reduction)
        )
    if "rgb_fine" in image_pred:
        psnr_fine = -10 * torch.log10(
            mse(image_pred["rgb_fine"].detach(), image_gt, valid_mask, reduction)
        )
...
class ConfusionMatrix:
    ...
    def add_batch(self, gt_image, pre_image, return_miou=False):
        ...
        ), f"gt shape {gt_image.shape} != pre shape {pre_image.shape}"
